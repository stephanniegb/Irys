...
const erc20_abi_1 = tslib_1.__importDefault(require("./erc20.abi"));
const walletConfig_1 = require("./walletConfig");
const starknetSigner = bundles_1.StarknetSigner;
...
const felt2hex = (felt) => '0x' + BigInt(felt).toString(16).padStart(64, '0');
exports.felt2hex = felt2hex;
...
class BaseSTRK20Token extends base_1.BaseNodeToken {
    constructor(config) {
        ...
        this.providerInstance = new starknet_1.RpcProvider({ nodeUrl: this.providerUrl });
        this.account = new starknet_2.Account(this.providerInstance, this._address, this.wallet, undefined, '0x3' // use version 3 so the gas token is $STRK
        );
    }
    async ready() {
        await this.getContract();
        const { publicKey, id } = await this.getAccountCompactPublicKey(this._address);
        this.signer = new bundles_1.StarknetSigner(this.providerInstance, this._address, this.wallet, id);
    ...
    async getContract() {
        if (!this.contractInstance) {
            this.contractInstance = new starknet_1.Contract(erc20_abi_1.default, this.contractAddress, this.providerInstance);
    ...
    async getTx(txId) {
        ...
        const transferEvent = receiptResponse.events.find((event) => event.keys.includes(TRANSFER_KEY) &&
            (0, exports.felt2hex)(event.from_address) == this.contractAddress);
        if (!transferEvent)
            throw new Error(`No transfer event found for contract ${this.contractAddress}`);
        const to = (0, exports.felt2hex)(transferEvent.data[1]);
        const amount = BigInt(transferEvent.data[2]);
        const from = (0, exports.felt2hex)(tracesResponse?.caller_address);
        return {
            from: from,
            to: to,
            blockHeight: receiptResponse?.block_number
                ? new bignumber_js_1.default(receiptResponse?.block_number)
                : undefined,
    ...
    async ownerToAddress(owner) {
        ...
        const contractId = (0, bundles_1.byteArrayToLong)(contractIdBuf);
    ...
    async getAccountCompactPublicKey(address, contractId) {
        if (contractId) {
            const config = walletConfig_1.KnownAccountContracts.get(contractId);
            if (config) {
                const contractInstance = new starknet_1.Contract(config.abi, address, this.providerInstance);
        ...
        for (const [id, config] of walletConfig_1.KnownAccountContracts.entries()) {
            const contractInstance = new starknet_1.Contract(config.abi, address, this.providerInstance);
            const publicKey = await contractInstance
                .call(config.selector, undefined, { parseResponse: false })
                .catch((_) => false);
    ...
    async getFee(amount, to, _multiplier) {
        ...
        const _amount = starknet_2.uint256.bnToUint256(amountBigNumber.toString());
        ...
        const result_to_hex = starknet_2.num.toHex(result);
        return new bignumber_js_1.default(result_to_hex);
    }
...
exports.default = BaseSTRK20Token;
