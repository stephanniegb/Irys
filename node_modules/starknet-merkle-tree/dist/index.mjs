...
var hashPair = (a, b, hashType = "Pedersen" /* Pedersen */) => BigInt(a) - BigInt(b) >= 0n ? num.toBigInt(hashType == "Pedersen" /* Pedersen */ ? ec.starkCurve.pedersen(b, a) : ec.starkCurve.poseidonHashMany([b, a])) : num.toBigInt(hashType == "Pedersen" /* Pedersen */ ? ec.starkCurve.pedersen(a, b) : ec.starkCurve.poseidonHashMany([a, b]));
...
function hashDataToHex(data, hashType = "Pedersen" /* Pedersen */) {
  ...
  return encode.addHexPrefix(hashType == "Pedersen" /* Pedersen */ ? ec.starkCurve.computeHashOnElements(aa).toString(16) : computePoseidonHashOnElements(aa).toString(16));
}
function hashDataToBigint(data, hashType = "Pedersen" /* Pedersen */) {
  ...
  return hashType == "Pedersen" /* Pedersen */ ? BigInt(ec.starkCurve.computeHashOnElements(aa)) : computePoseidonHashOnElements(aa);
}
...
function renderMerkleTree(tree) {
  ...
  while (stack.length > 0) {
    ...
    lines.push(
      ...
      path.slice(0, -1).map((p) => ["   ", "\u2502  "][p]).join("") + path.slice(-1).map((p) => ["\u2514\u2500 ", "\u251C\u2500 "][p]).join("") + i + ") " + encode.addHexPrefix(tree[i].toString(16))
    );
